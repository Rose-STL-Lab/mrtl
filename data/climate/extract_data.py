...
def get_en4_single_year(f, path):
    '''Function downloads single year of EN4 data, extracts surface level, and saves as NetCDF file
    ...
    cdo.sellevel(5.0215898, input=f'-selname,salinity,temperature {temp_path}', output=out_path) # get top level
...
def cleanup(path):
    ...
    types = ('*.stx', '*.csv', '*.info.txt', '*.prj', '*.xml', '*.hdr', '*.bil') # file types to remove
...
def get_prism_from_zip(f, path):
    '''Function extracts data from .zip files and saves to .nc format (with monthly files)'''
...
def get_prism_single_month(year, month, path):
    ...
    in_path = os.path.join(path, f'PRISM_ppt_stable_4kmM{suffix}_{year}{month}_bil.bil')
    d = xr.open_rasterio(in_path)
    d = d.rename({'x':'lon', 'y':'lat','band':'time'}).assign_coords(time=[datetime(int(year),int(month),1)])
...
#     #################### EN4 DATA ####################
    ...
    cdo.mergetime(input=os.path.join(path,'en4*.nc'), output=os.path.join(path,'all_en4.nc'))
    ...
    for f in prism_files:
        ...
        if year <= 1980:
            ...
            f_local = os.path.join(path, f'PRISM_ppt_stable_4kmM2_{year}_all_bil.zip')
        else:
        ...
        # merge files
        cdo.mergetime(input=os.path.join(path, 'prism*.nc'), output=os.path.join(path, 'all_prism.nc'))
        ...
        os.rename(os.path.join(path, 'all_prism.nc'), os.path.join(path, 'prism.nc')) # rename full data
        
    os.rename(os.path.join(path, 'prism.nc'), os.path.join(path, 'all_prism.nc')) # rename full data